...
Connection.prototype._apiRequest = function(opts, callback) {
  ...
  return request(ropts, function(err, res, body) {
    ...
    if(!body && res.headers && res.headers.error) {
      return callback(new NForceError.ApiCallFailure('Response has error in header with empty body', res.headers.error, res.statusCode), null);
    }
    ...
    var processResponse = function(data) {
      ...
      if (data) {
        ...
        if (Array.isArray(data) && data.length > 0) {
          e = new NForceError.ApiCallFailure(data[0].message, data[0].errorCode, res.statusCode);
        } else {
